...
while True:
    ...
    if ids is not None and len(ids) > 0:
        for i, corner in enumerate(corners):
            ...
            euler_angles = rotation_matrix_to_euler_angles(R) #Calculate Euler angles from the rotation matrix (using func above)
            ...
            cv2.putText(frame, f'distance: {distance:.2f}', (x_offset, y_offset - 40), font, 0.5, (0, 255, 0), 1, cv2.LINE_AA) #Displaying Distance
            cv2.putText(frame, f'coordinates: ({x:.2f}, {y:.2f}, {z:.2f})', (x_offset, y_offset - 20), font, 0.5, (0, 255, 0), 1, cv2.LINE_AA) #Displaying Position
            cv2.putText(frame, f'pitch: {pitch:.2f}, yaw: {yaw:.2f}, roll: {roll:.2f}', (x_offset, y_offset - 60), font, 0.5, (0, 255, 0), 1, cv2.LINE_AA) #Displaying Orientation
